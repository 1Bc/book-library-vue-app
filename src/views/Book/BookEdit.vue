...
let book = await BookService.getBook(props.index);
...
async function submitBook() {
  ...
  await BookService.updateBook(book.id,request);
...
async function deleteBook(){
  ...
  if(confirmation){
    await BookService.deleteBook(props.index);
    bus.$emit('book-created', {});
    goToBooks();
  }

}

const isSubmitDisabled = computed(() => !book.title || !book.pages || !book.author);
...
<template>
  ...
  <form class="container mb-3" action="">
    ...
    <div class="row justify-content-center">
      ...
      <div class="row justify-content-center">
        <p class="text-center" v-if="book.author">{{ book.author.firstName + ' ' + book.author.lastName }}</p>
